"""
Data migration: seed всіх зачісок у БД.
Запусти: python manage.py migrate
"""
...
def seed_hairstyles(apps, schema_editor):
    ...
    for name, category, description in HAIRSTYLES:
        hs, was_created = Hairstyle.objects.get_or_create(
            name=name,
            defaults={'name_en': '', 'description': description, 'category': category, 'master_count': 0}
        )
        if was_created:
            created += 1
        else:
            ...
            if changed:
                hs.save()
                updated += 1
    print(f'\n  [migration] Зачіски: створено {created}, оновлено {updated}')


def unseed_hairstyles(apps, schema_editor):
    pass  # не видаляємо при rollback
